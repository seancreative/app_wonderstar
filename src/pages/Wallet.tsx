...
const WalletTopup: React.FC = () => {
  ...
  const handleTopup = async () => {
    ...
    try {
      ...
      if (!shopOrder?.order_number) {
        console.error('[Topup] Order created but no order_number:', shopOrder);
        throw new Error('Order created but missing order number');
      }
      ...
      const { data: paymentTx, error: paymentError } = await supabase
        ...
        .insert({
          order_id: shopOrder.order_number,
          user_id: user.id,
          amount: selectedPackage.amount,
          payment_method: selectedPayment,
