/*
  ...
  1. **users** - Add address fields for Fiuu customer creation
...
-- =====================================================
-- UPDATE USERS TABLE - Add Address Fields
-- =====================================================
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE users ADD COLUMN country text DEFAULT 'MY';
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS payment_transactions (
  ...
  CONSTRAINT valid_payment_method CHECK (payment_method IN ('credit', 'debit', 'fpx', 'grabpay', 'tng', 'boost', 'ewallet'))
);
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'shop_orders' AND column_name = 'payment_method'
  ) THEN
    ...
    ALTER TABLE shop_orders ADD CONSTRAINT valid_payment_method
      CHECK (payment_method IN ('wonderstars', 'credit', 'debit', 'fpx', 'grabpay', 'tng', 'boost'));
  END IF;
END $$;
