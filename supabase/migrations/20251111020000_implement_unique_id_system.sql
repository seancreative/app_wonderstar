...
ALTER TABLE outlets ADD COLUMN IF NOT EXISTS display_id TEXT UNIQUE;

-- Add display_id column to staff_passcodes table (Staff ID)
ALTER TABLE staff_passcodes ADD COLUMN IF NOT EXISTS display_id TEXT UNIQUE;

-- Add display_id column to rewards table (Rewards ID)
ALTER TABLE rewards ADD COLUMN IF NOT EXISTS display_id TEXT UNIQUE;
...
-- For outlets: assign O001, O002 based on creation order
DO $$
DECLARE
...
-- For staff: assign S001, S002 based on creation order
DO $$
DECLARE
  staff_record RECORD;
  counter INTEGER := 1;
...
-- For rewards: assign R001, R002 based on creation order
DO $$
DECLARE
...
-- For users/customers: assign customer IDs
DO $$
DECLARE
  user_record RECORD;
BEGIN
